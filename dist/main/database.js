...
exports.CampaignDatabase = void 0;
...
class CampaignDatabase {
    ...
    `);
        ...
        const versionRow = this.db.prepare('SELECT value FROM campaign_meta WHERE key = ?').get('version');
        if (!versionRow) {
    ...
    // ============================================
    getCampaignMeta() {
        const rows = this.db.prepare('SELECT key, value FROM campaign_meta').all();
        const meta = {};
        rows.forEach(row => {
    ...
    // ============================================
    getTemplates() {
        const rows = this.db.prepare('SELECT * FROM card_templates ORDER BY name').all();
        return rows.map(row => ({
            ...row,
    ...
    getTemplate(id) {
        const row = this.db.prepare('SELECT * FROM card_templates WHERE id = ?').get(id);
        if (!row)
            return null;
        return {
            ...row,
    ...
    `).run(id, data.name, data.icon, data.color, JSON.stringify(data.field_definitions), now, now);
        this.updateCampaignMeta({}); // Update modified_at
    ...
    updateTemplate(id, updates) {
      ...
      WHERE id = ?
    `).run(updated.name, updated.icon, updated.color, JSON.stringify(updated.field_definitions), now, id);
        this.updateCampaignMeta({});
        return this.getTemplate(id);
    }
    deleteTemplate(id) {
        // Check if any cards use this template
        const count = this.db.prepare('SELECT COUNT(*) as count FROM cards WHERE template_id = ?').get(id);
        if (count.count > 0) {
    ...
    // ============================================
    getLists() {
        const rows = this.db.prepare('SELECT * FROM lists ORDER BY position').all();
        return rows.map(row => ({
            ...row,
            collapsed: Boolean(row.collapsed),
        }));
    ...
    `).run(id, data.name, data.position, now);
        this.updateCampaignMeta({});
        return this.db.prepare('SELECT * FROM lists WHERE id = ?').get(id);
    }
  ...
  `).all(listId);
    ...
    createCard(data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, data.list_id, data.template_id, data.name, JSON.stringify(data.field_values), data.position, data.parent_folder_id || null, data.folder_level || 0, data.is_folder ? 1 : 0, data.is_expanded !== undefined ? (data.is_expanded ? 1 : 0) : 1, now, now);
        this.updateCampaignMeta({});
    ...
    reorderCards(listId, cardId, newPosition) {
        const transaction = this.db.transaction(() => {
            // Get all cards in the list
            const cards = this.db.prepare('SELECT id, position FROM cards WHERE list_id = ? ORDER BY position').all(listId);
    ...
    getLinkedCards(cardId, fieldKey) {
        const rows = this.db.prepare(`
      SELECT
        c.*,
        t.name as template_name,
    ...
    createLink(sourceCardId, targetCardId, fieldKey) {
        const id = (0, uuid_1.v4)();
        const now = new Date().toISOString();
        this.db.prepare(`
    ...
    `).run(id, sourceCardId, targetCardId, fieldKey, now);
        this.updateCampaignMeta({});
        return this.db.prepare('SELECT * FROM card_links WHERE id = ?').get(id);
    }
    deleteLink(id) {
        this.db.prepare('DELETE FROM card_links WHERE id = ?').run(id);
        this.updateCampaignMeta({});
    }
    deleteLinksForField(cardId, fieldKey) {
    ...
    getImage(id) {
        const row = this.db.prepare('SELECT * FROM image_blobs WHERE id = ?').get(id);
        return row || null;
    }
    deleteImage(id) {
...
exports.CampaignDatabase = CampaignDatabase;
